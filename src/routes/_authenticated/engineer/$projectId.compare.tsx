...
interface Row { id: string; description: string; qty: number; unit_cost: number; amount: number; actual_cost: number | null }
...
function ComparePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data: q } = await supabase.from("quotations").select("id").eq("project_id", projectId).order("created_at", { ascending: false }).maybeSingle();
    if (q) {
  ...
  async function updateActual(id: string, v: number) {
    const { error } = await supabase.from("quotation_items").update({ actual_cost: v }).eq("id", id);
  ...
  return (
    <div className="p-4 md:p-8 fade-in max-w-4xl mx-auto pb-24">
      ...
      {loading ? <p className="mt-6 text-sm text-muted-foreground">Loading…</p>
        : rows.length === 0 ? <p className="mt-6 text-sm text-muted-foreground">No quotation yet for this project.</p>
        : (
          <div className="mt-6 rounded-lg border overflow-x-auto bg-card">
            <table className="w-full text-sm">
              <thead className="bg-surface text-left">
                <tr><th className="p-3">Item</th><th className="p-3 text-right">Quoted</th><th className="p-3 text-right w-40">Actual</th><th className="p-3 text-right">Diff</th></tr>
              </thead>
              <tbody>
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="p-1"><Input type="number" step="0.01" defaultValue={r.actual_cost ?? 0} onBlur={(e) => updateActual(r.id, Number(e.target.value))} className="text-right" /></td>
                      <td className={`p-3 text-right tabular-nums ${d > 0 ? "text-red-700" : d < 0 ? "text-green-700" : ""}`}>{d.toFixed(2)}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="border-t bg-surface font-medium">
                  ...
                  <td className={`p-3 text-right tabular-nums ${diff > 0 ? "text-red-700" : diff < 0 ? "text-green-700" : ""}`}>{diff.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
