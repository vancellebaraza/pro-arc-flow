...
interface Item { id?: string; description: string; unit: string; qty: number; unit_cost: number; amount: number; actual_cost?: number | null }
...
function QuotationPage() {
  ...
  const [items, setItems] = useState<Item[]>([{ description: "", unit: "pcs", qty: 1, unit_cost: 0, amount: 0 }]);
  ...
  const load = useCallback(async () => {
    ...
    if (q) {
      ...
      const meta = (q.meta ?? {}) as { bill_to?: string; recipient?: string; for_text?: string; authorised_by?: string; date?: string };
      ...
      const rows = (it ?? []) as Array<{ id: string; description: string; unit: string | null; qty: number; unit_cost: number; amount: number; actual_cost: number | null }>;
      if (rows.length) setItems(rows.map((r) => ({ id: r.id, description: r.description, unit: r.unit ?? "", qty: Number(r.qty), unit_cost: Number(r.unit_cost), amount: Number(r.amount), actual_cost: r.actual_cost == null ? null : Number(r.actual_cost) })));
    } else {
  ...
  async function save(newStatus?: "draft" | "sent") {
    ...
    try {
      ...
      const meta = { bill_to: billTo, recipient, for_text: forText, authorised_by: authorisedBy, date };
      const payload = {
      ...
      if (newStatus === "sent") {
        await supabase.from("projects").update({ status: "quoted" as const }).eq("id", projectId);
      }
  ...
  function exportPdf() {
    generateQuotationPdf({
      ...
      to: recipient,
      forText,
      items: items.map((i) => ({ description: i.description, unit: i.unit, qty: i.qty, unit_cost: i.unit_cost, amount: i.amount })),
      labour: Number(labour || 0),
      subtotal,
      grandTotal,
      authorisedBy,
  ...
  return (
    <div className="p-4 md:p-8 fade-in max-w-5xl mx-auto pb-24">
      ...
      <section className="mt-6 grid md:grid-cols-2 gap-4">
        <div><Label>Bill To</Label><Input value={billTo} onChange={(e) => setBillTo(e.target.value)} placeholder="Client / company name" /></div>
        <div><Label>Quotation No</Label><Input value={quoteNo} onChange={(e) => setQuoteNo(e.target.value)} /></div>
        <div><Label>Date</Label><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></div>
        <div><Label>To (recipient)</Label><Input value={recipient} onChange={(e) => setRecipient(e.target.value)} placeholder="Recipient name / contact" /></div>
        <div className="md:col-span-2"><Label>For (scope / purpose)</Label><Input value={forText} onChange={(e) => setForText(e.target.value)} placeholder="Short scope description" /></div>
      </section>

      <section className="mt-8">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold tracking-tight">Line Items</h2>
          <Button variant="outline" size="sm" onClick={() => setItems((a) => [...a, { description: "", unit: "pcs", qty: 1, unit_cost: 0, amount: 0 }])}><Plus className="h-4 w-4 mr-1" />Add row</Button>
        </div>
            ...
            <tbody>
              {items.map((it, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td className="p-1"><Input value={it.unit} onChange={(e) => update(i, { unit: e.target.value })} placeholder="pcs / m / hr" /></td>
                  ...
                  <td className="p-1"><Button type="button" variant="ghost" size="icon" onClick={() => setItems((a) => a.filter((_, j) => j !== i))}><Trash2 className="h-4 w-4" /></Button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="mt-6 grid md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="mt-4">
            ...
            <Input value={authorisedBy} onChange={(e) => setAuthorisedBy(e.target.value)} placeholder="Approver name" />
          </div>
        </div>
        <div className="rounded-lg border bg-surface p-4 self-start space-y-2">
          <div className="flex items-center gap-2">
            <Label className="flex-1">Labour (KES)</Label>
            <Input type="number" step="0.01" value={labour} onChange={(e) => setLabour(Number(e.target.value))} className="w-32 text-right" />
          </div>
          <div className="flex justify-between text-sm"><span>Sub-total</span><strong className="tabular-nums">{subtotal.toFixed(2)}</strong></div>
          <div className="flex justify-between text-base"><span>Grand Total (KES)</span><strong className="tabular-nums">{grandTotal.toFixed(2)}</strong></div>
      ...
      <section className="mt-8 rounded-lg border bg-card p-5">
        ...
        <div className="mt-3 grid sm:grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <div><span className="text-muted-foreground">Bank:</span> <strong>{BANK_DETAILS.bank}</strong></div>
          <div><span className="text-muted-foreground">Account Name:</span> <strong>{BANK_DETAILS.account_name}</strong></div>
          <div><span className="text-muted-foreground">Branch:</span> <strong>{BANK_DETAILS.branch}</strong></div>
          <div><span className="text-muted-foreground">Bank Code:</span> <strong>{BANK_DETAILS.bank_code}</strong></div>
          <div><span className="text-muted-foreground">Account No:</span> <strong>{BANK_DETAILS.account_number}</strong></div>
          <div><span className="text-muted-foreground">Swift Code:</span> <strong>{BANK_DETAILS.swift_code}</strong></div>
        </div>
      </section>
      ...
      <div className="mt-8 flex flex-wrap justify-end gap-2">
        ...
        <Button disabled={saving} onClick={() => save("sent")}><Send className="h-4 w-4 mr-1" />Send to client</Button>
      </div>
    </div>
  );
}
