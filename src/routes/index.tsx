...
function Landing() {
  ...
  async function submitInquiry() {
    ...
    const message = encodeURIComponent(
      `Hello FusionPro, I am ${inquiryName.trim()} and I would like to enquire about ${inquiryService.label}. Email: ${inquiryEmail.trim() || "not provided"}. Phone: ${inquiryPhone.trim() || "not provided"}. Message: ${inquiryMessage.trim() || "No additional details."}`,
    );
          ...
          <div className="md:hidden">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <DropdownMenuItem asChild>
                  <Link to="/auth" className="flex w-full items-center justify-between">
                    Sign in
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                </DropdownMenuItem>
          ...
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center">
            <div className="relative">
              ...
              <p className="mt-6 max-w-2xl text-base md:text-lg text-muted-foreground leading-relaxed">
                FusionPro Limited delivers reliable repair, maintenance,renovation, and home
                improvement services for home owners,landlords,businesses, and property managers.
                From small fixes to complete renovations, we handle every project with quality
        ...
        <DialogContent className="max-w-3xl p-0 overflow-hidden">
          {selectedService && (
            <>
              ...
              <div className="p-6">
                <DialogHeader>
                  <DialogTitle className="text-2xl">{selectedService.label}</DialogTitle>
                </DialogHeader>

                <p className="mt-4 text-muted-foreground leading-7">{selectedService.desc}</p>

                {/* Optional extra details */}
                <div className="mt-6 flex items-center gap-2">
                  <selectedService.icon className="h-5 w-5 text-primary" />
                  <span className="font-medium">FusionPro Services</span>
                </div>
